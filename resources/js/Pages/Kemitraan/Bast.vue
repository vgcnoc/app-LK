...
const statusColor = (status) => {
    const map = {
        'Menunggu Tanda Tangan': 'bg-amber-100 text-amber-700',
            ...
            <div class="hidden sm:flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs sm:text-sm text-slate-500 mt-1">
                        Dokumen serah terima pemasangan internet ke pelanggan.
                    </p>
                </div>
            ...
            <div class="max-w-full mx-auto space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-2xl font-black text-slate-800">{{ bastList.filter(b => b.status === 'Menunggu Tanda Tangan').length }}</p>
                            <p class="text-xs text-slate-500">Menunggu TTD</p>
                        </div>
                    </div>
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-2xl font-black text-slate-800">{{ bastList.filter(b => b.status === 'Selesai').length }}</p>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-2xl font-black text-slate-800">{{ bastList.length }}</p>
                            <p class="text-xs text-slate-500">Total BAST</p>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left text-sm text-slate-600">
                            <thead class="text-xs uppercase text-slate-400 border-b border-slate-100 bg-slate-50/50">
                                <tr>
                                    ...
                                    <th class="py-4 px-6 font-semibold">Tgl Serah Terima</th>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <template v-if="bastList.length > 0">
                                    <tr v-for="bast in bastList" :key="bast.id" class="hover:bg-slate-50 transition-colors">
                                        <td class="py-4 px-6 font-mono font-medium text-indigo-600 text-sm">{{ bast.nomor }}</td>
                                        <td class="py-4 px-6">
                                            <p class="font-medium text-slate-800">{{ bast.nama_pelanggan }}</p>
                                            <p class="text-xs text-slate-400 mt-0.5">{{ bast.no_hp }}</p>
                                        </td>
                                        <td class="py-4 px-6 max-w-xs truncate">{{ bast.alamat }}</td>
                                        <td class="py-4 px-6">
                                            <span class="px-2.5 py-1 rounded-lg text-xs font-semibold bg-indigo-50 text-indigo-700">{{ bast.paket }}</span>
                                        </td>
                                ...
                                <tr v-else>
                                    <td colspan="7" class="py-16 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-sm text-slate-500">Belum ada berita acara serah terima</p>
                                            <p class="text-xs text-slate-400">BAST akan otomatis dibuat setelah pemasangan selesai</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
